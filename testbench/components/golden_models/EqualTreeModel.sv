...
class EqualTreeModel #(type T);
    ...
    task automatic run();
        ...
        forever begin
            ...
            while(io_obj_in.equal_tree_io_in_q.size() > 0) begin
                equal_tree_io_in = io_obj_in.equal_tree_io_in_q.pop_front();

                if(io_obj_in.idle.pop_front()) begin
                    continue;
                end

                equal_tree_io_out.eq_o = this.equal_data(
                    equal_tree_io_in.data_a_i,
                    equal_tree_io_in.data_b_i
                );
                
                io_obj_out.error_state.push_back(this.error_state);
                io_obj_out.equal_tree_io_out_q.push_back(equal_tree_io_out);
            end
