...
class EqualTreeDriver #(type T, type I);
    ...
    task automatic drive(T io_obj);
        ...
        while(io_obj.equal_tree_io_in_q.size() > 0) begin
            
            equal_tree_io_in = io_obj.equal_tree_io_in_q.pop_front();
            ...
            if(io_obj.equal_tree_io_in_q.size() == 0) begin
                inf.end_sequence <= 1;
            end else begin
                inf.end_sequence <= 0;
            end
            ...
            if(io_obj.idle.pop_front()) begin
                ...
                inf.data_a_i <= equal_tree_io_in.data_a_i;
                inf.data_b_i <= equal_tree_io_in.data_b_i;
                inf.idle     <= 0;
            end
        end
