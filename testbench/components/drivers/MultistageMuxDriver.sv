...
class MultistageMuxDriver #(type T, type I);
    ...
    task automatic drive(T io_obj);
        ...
        while(io_obj.multistage_mux_io_in_q.size() > 0) begin
            
            multistage_mux_io_in = io_obj.multistage_mux_io_in_q.pop_front();
            ...
            if(io_obj.multistage_mux_io_in_q.size() == 0) begin
                inf.end_sequence <= 1;
            end else begin
                inf.end_sequence <= 0;
            end
            ...
            if(io_obj.idle.pop_front()) begin
                ...
                inf.data_i <= multistage_mux_io_in.data_i;
                inf.sel_i  <= multistage_mux_io_in.sel_i;
                inf.idle   <= 0;
            end
        end
