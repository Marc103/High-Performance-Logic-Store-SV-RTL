...
class QueuePushCouplerGenerator #(type T);
    ...
    task automatic run();
        ...
        // Fill while the downstream queue is blocked, crossing the reservoir
        // watermark and consuming all configured backpressure storage.
        for(int entry = 0; entry < T::RESERVOIR_ENTRIES; entry++) begin
            add_io(io_obj, 0, 0, 1, 1, 1);
        end
        add_io(io_obj, 0, 0, 0, 1, 1);
        ...
        for(int cycle = 0; cycle < 48; cycle++) begin
            ...
            add_io(io_obj, 0, 0, ((cycle % 4) != 3), full, lookahead);
        end
