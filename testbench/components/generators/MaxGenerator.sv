...
class MaxGenerator #(type T);
    ...
    task automatic add_io(
        ...
        max_io_in.data_a_i = data_a;
        max_io_in.data_b_i = data_b;

        io_obj.max_io_in_q.push_back(max_io_in);
    ...
    task automatic run();
        ...
        // signed-sensitive cases when SIGNED is enabled
        add_io(io_obj, 0, {1'b1, {(T::DATA_WIDTH - 1){1'b0}}}, '0);
        add_io(io_obj, 0, {1'b1, {(T::DATA_WIDTH - 1){1'b1}}}, 'h1);
        add_io(io_obj, 0, {1'b0, {(T::DATA_WIDTH - 1){1'b1}}}, {1'b1, {(T::DATA_WIDTH - 1){1'b0}}});
