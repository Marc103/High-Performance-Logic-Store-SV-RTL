...
class EqualMonitor #(type T, type I);
    `EQUAL_IO_OUT_STRUCT
    ...
    equal_io_out_t [T::LATENCY : 0] equal_out_l;
        ...
        forever begin
            ...
            if(T::LATENCY == 0) begin
                if(inf.start_sequence || active_sequence) begin
                    if(!inf.idle) begin
                        equal_io_out.eq_o = inf.eq_o;
                        io_obj.equal_io_out_q.push_back(equal_io_out);
                    end
            ...
            equal_out_l[0].eq_o <= inf.eq_o;
            
            // output pipelining
            for(int i = 1; i <= T::LATENCY; i++) begin
                if((i == T::LATENCY) && (v_l[i - 1])) begin
                    equal_out_l[i].eq_o <= inf.eq_o;
                end else begin 
                    equal_out_l[i].eq_o <= equal_out_l[i - 1].eq_o;
                end 
            end
            
            // for valid commands
            if(v_l[T::LATENCY]) begin
                io_obj.equal_io_out_q.push_back(equal_out_l[T::LATENCY]);
