...
class EqualTreeMonitor #(type T, type I);
    ...
    equal_tree_io_out_t [T::LATENCY : 0] equal_tree_out_l;
    ...
    task automatic run();
        T io_obj;
        equal_tree_io_out_t equal_tree_io_out;
        ...
        forever begin
            ...
            if(T::LATENCY == 0) begin
                if(inf.start_sequence || active_sequence) begin
                    if(!inf.idle) begin
                        equal_tree_io_out.eq_o = inf.eq_o;
                        io_obj.equal_tree_io_out_q.push_back(equal_tree_io_out);
                    end
            ...
            equal_tree_out_l[0].eq_o <= inf.eq_o;
            
            // output pipelining
            for(int i = 1; i <= T::LATENCY; i++) begin
                if((i == T::LATENCY) && (v_l[i - 1])) begin
                    equal_tree_out_l[i].eq_o <= inf.eq_o;
                end else begin 
                    equal_tree_out_l[i].eq_o <= equal_tree_out_l[i - 1].eq_o;
                end 
            end
            
            // for valid commands
            if(v_l[T::LATENCY]) begin
                io_obj.equal_tree_io_out_q.push_back(equal_tree_out_l[T::LATENCY]);
