...
class QueueScoreboard #(type T);
    ...
    task automatic run();
        T dut_io_obj;
        T model_io_obj;
        queue_io_out_t dut_queue_io_out;
        queue_io_out_t model_queue_io_out;
        ...
        forever begin
            ...
            assert(dut_io_obj.queue_io_out_q.size() == model_io_obj.queue_io_out_q.size())  begin
                $display("%d %d : Output sequence sizes are the same \n DUT : %d, Model :  %d",
                        obj_iter,
                        seq_iter, 
                        dut_io_obj.queue_io_out_q.size(), 
                        model_io_obj.queue_io_out_q.size());
            end else begin
            ...
            seq_iter = 0;
            while(dut_io_obj.queue_io_out_q.size() > 0) begin
                dut_queue_io_out = dut_io_obj.queue_io_out_q.pop_front();

                model_queue_io_out = model_io_obj.queue_io_out_q.pop_front();
