...
class MaxScoreboard #(type T);
    ...
    task automatic run();
        T dut_io_obj;
        T model_io_obj;
        max_io_out_t dut_max_io_out;
        max_io_out_t model_max_io_out;
        ...
        forever begin
            ...
            assert(dut_io_obj.max_io_out_q.size() == model_io_obj.max_io_out_q.size())  begin
                $display("%d %d : Output sequence sizes are the same \n DUT : %d, Model :  %d",
                        obj_iter,
                        seq_iter, 
                        dut_io_obj.max_io_out_q.size(), 
                        model_io_obj.max_io_out_q.size());
            end else begin
            ...
            seq_iter = 0;
            while(dut_io_obj.max_io_out_q.size() > 0) begin
                dut_max_io_out = dut_io_obj.max_io_out_q.pop_front();

                model_max_io_out = model_io_obj.max_io_out_q.pop_front();
                ...
                assert(dut_max_io_out === model_max_io_out)
                    else $error("%d %d: DUT does not match Model \n DUT : %p \n Model : %p", obj_iter, seq_iter, dut_max_io_out, model_max_io_out);
