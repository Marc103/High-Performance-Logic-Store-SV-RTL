...
class ReservoirScoreboard #(type T);
    ...
    task automatic compare_output(
        input reservoir_io_out_t dut_output,
        input reservoir_io_out_t model_output,
    ...
    task automatic run();
        T dut_io_obj;
        T model_io_obj;
        reservoir_io_out_t dut_output;
        reservoir_io_out_t model_output;
        ...
        forever begin
            ...
            if(dut_io_obj.reservoir_io_out_q.size() !=
               model_io_obj.reservoir_io_out_q.size()) begin
                $error("Output sequence size mismatch, DUT=%0d Model=%0d",
                    dut_io_obj.reservoir_io_out_q.size(),
                    model_io_obj.reservoir_io_out_q.size());
                this.mismatch_count++;
            end

            while((dut_io_obj.reservoir_io_out_q.size() > 0) &&
                  (model_io_obj.reservoir_io_out_q.size() > 0)) begin
                dut_output = dut_io_obj.reservoir_io_out_q.pop_front();
                model_output = model_io_obj.reservoir_io_out_q.pop_front();
                ...
                compare_output(dut_output, model_output, this.checked_cycles);
                this.checked_cycles++;
            end
            ...
            if(dut_io_obj.end_last_sequence || model_io_obj.end_last_sequence) begin
                ...
                if(this.mismatch_count == 0) begin
                    $display("Reservoir scoreboard passed %0d checked cycles",
                        this.checked_cycles);
                    $finish;
                end else begin
                    $fatal(1, "Reservoir scoreboard failed with %0d mismatches",
                        this.mismatch_count);
                end
            end
        end
