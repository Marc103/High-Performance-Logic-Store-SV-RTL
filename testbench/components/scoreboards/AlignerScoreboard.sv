...
class AlignerScoreboard #(type T);
    ...
    task automatic run();
        T dut_io_obj;
        T model_io_obj;
        aligner_io_out_t dut_aligner_io_out;
        aligner_io_out_t model_aligner_io_out;
        ...
        forever begin
            ...
            assert(dut_io_obj.aligner_io_out_q.size() == model_io_obj.aligner_io_out_q.size())
                else $error(
                    "%d: Output sequence sizes differ, DUT: %d, Model: %d",
                    obj_iter,
                    dut_io_obj.aligner_io_out_q.size(),
                    model_io_obj.aligner_io_out_q.size()
                );

            seq_iter = 0;
            while(dut_io_obj.aligner_io_out_q.size() > 0) begin
                dut_aligner_io_out = dut_io_obj.aligner_io_out_q.pop_front();
                model_aligner_io_out = model_io_obj.aligner_io_out_q.pop_front();
                ...
                assert(dut_aligner_io_out === model_aligner_io_out)
                    else $error(
                        "%d %d: DUT does not match Model\nDUT: %p\nModel: %p",
                        obj_iter,
                        seq_iter,
                        dut_aligner_io_out,
                        model_aligner_io_out
                    );
            ...
            if(dut_io_obj.end_last_sequence) begin
                $display("Aligner scoreboard passed %0d comparisons", seq_iter);
                $finish;
            end
