...
class QueueTrainScoreboard #(type T);
    ...
    task automatic run();
        T dut_io_obj;
        T model_io_obj;
        queue_train_io_out_t dut_output;
        queue_train_io_out_t model_output;
        ...
        forever begin
            ...
            if(dut_io_obj.queue_train_io_out_q.size() !=
               model_io_obj.queue_train_io_out_q.size()) begin
                $display("ERROR: Transfer count mismatch, DUT=%0d Model=%0d",
                    dut_io_obj.queue_train_io_out_q.size(),
                    model_io_obj.queue_train_io_out_q.size());
                this.mismatch_count++;
            end

            while((dut_io_obj.queue_train_io_out_q.size() > 0) &&
                  (model_io_obj.queue_train_io_out_q.size() > 0)) begin
                dut_output = dut_io_obj.queue_train_io_out_q.pop_front();
                model_output = model_io_obj.queue_train_io_out_q.pop_front();
            ...
            if(dut_io_obj.end_last_sequence || model_io_obj.end_last_sequence) begin
                ...
                if(this.mismatch_count == 0) begin
                    $display("QueueTrain scoreboard passed %0d ordered transfers",
                        this.checked_transfers);
                    $finish;
                end else begin
                    $fatal(1, "QueueTrain scoreboard failed with %0d mismatches",
                        this.mismatch_count);
                end
            end
        end
