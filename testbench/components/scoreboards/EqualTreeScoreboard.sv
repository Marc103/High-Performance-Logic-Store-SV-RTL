...
class EqualTreeScoreboard #(type T);
    ...
    task automatic run();
        T dut_io_obj;
        T model_io_obj;
        equal_tree_io_out_t dut_equal_tree_io_out;
        equal_tree_io_out_t model_equal_tree_io_out;
        ...
        forever begin
            ...
            assert(dut_io_obj.equal_tree_io_out_q.size() == model_io_obj.equal_tree_io_out_q.size())  begin
                $display("%d %d : Output sequence sizes are the same \n DUT : %d, Model :  %d",
                        obj_iter,
                        seq_iter, 
                        dut_io_obj.equal_tree_io_out_q.size(), 
                        model_io_obj.equal_tree_io_out_q.size());
            end else begin
            ...
            seq_iter = 0;
            while(dut_io_obj.equal_tree_io_out_q.size() > 0) begin
                dut_equal_tree_io_out = dut_io_obj.equal_tree_io_out_q.pop_front();

                model_equal_tree_io_out = model_io_obj.equal_tree_io_out_q.pop_front();
                ...
                assert(dut_equal_tree_io_out === model_equal_tree_io_out)
                    else $error("%d %d: DUT does not match Model \n DUT : %p \n Model : %p", obj_iter, seq_iter, dut_equal_tree_io_out, model_equal_tree_io_out);
